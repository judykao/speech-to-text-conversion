...
for i in range(len(raw_folders)):
    ...
    noisy_dir=r'/working/CNN/Tasks/%s/Recorded'%raw_folders[i]
    # noisy_dir=r'/working/CNN/Test/%s/Recorded'%raw_folders[i]
    noisy_filename,noisy_audio_int=sampling(noisy_dir)

    clean_dir=r'/working/CNN/Tasks/%s/Clean'%raw_folders[i]
    ...
    for j in range(n):
        noisy_aligned, clean_aligned = adjust_signal(noisy_audio_int[j], clean_audio_int[j])
        print(f'wav {j}/{n} in {raw_folders[i]} is alligned.')
